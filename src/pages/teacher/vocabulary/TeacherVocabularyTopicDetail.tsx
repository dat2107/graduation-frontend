...
const emptyForm: CreateVocabWordRequest = {
  word: '', phonetic: '', meaning: '', example: '', level: 'A1',
}
...
const TeacherVocabularyTopicDetail = () => {
  ...
  useEffect(() => {
    const fetchTopic = async () => {
      try {
        const resp = await VocabularyService.getTopicsForManage({ page: 0, size: 1, search: '' })
        // We'll get the topic info from the word list header or a separate call
  ...
  const handleSubmit = async () => {
    if (!formData.word.trim() || !formData.meaning.trim()) {
      setAlert({ type: 'error', message: 'Từ vựng và nghĩa không được để trống' })
      return
    }
    setFormLoading(true)
    try {
      if (editingWord) {
        ...
        setAlert({ type: 'success', message: `Đã cập nhật "${formData.word}"` })
      } else {
      ...
      setAlert({ type: 'error', message: editingWord ? 'Không thể cập nhật' : 'Không thể thêm từ' })
    } finally {
  ...
  return (
    <Box p="lg">
      {/* Breadcrumb */}
      <Breadcrumbs mb="md">
        <Anchor component={Link} to="/teacher/vocabulary" size="sm">QL Từ vựng</Anchor>
      ...
      <Group justify="space-between" mb="lg">
        <Group>
          <ActionIcon variant="subtle" onClick={() => navigate('/teacher/vocabulary')}>
            <IconArrowLeft size={20} />
          </ActionIcon>
          <Stack gap={2}>
            <Title order={2}>Từ vựng trong chủ đề #{topicId}</Title>
            <Text c="dimmed" size="sm">{totalItems} từ</Text>
          </Stack>
        </Group>
            ...
            <Table.Tbody>
              {loading ? (
                ...
                words.map((w) => (
                  <Table.Tr key={w.id}>
                    <Table.Td>
                      <Text size="sm" fw={600}>{w.word}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm" c="dimmed">{w.phonetic || '—'}</Text>
                    </Table.Td>
                    <Table.Td ta="center">
                      {w.partOfSpeech ? (
                        <Badge variant="outline" size="xs">{w.partOfSpeech}</Badge>
                      ) : (
                        <Text size="xs" c="dimmed">—</Text>
                      )}
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm" lineClamp={1} maw={200}>{w.meaning}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="xs" c="dimmed" lineClamp={1} maw={200} fs="italic">
                        {w.example || '—'}
                      </Text>
                    </Table.Td>
                    <Table.Td ta="center">
                      <Badge variant="light" color={LEVEL_COLORS[w.level] || 'gray'} size="xs">
                        {w.level}
                      </Badge>
                    </Table.Td>
                    <Table.Td ta="center">
                      {actionLoading === w.id ? (
                        ...
                        <Menu shadow="md" width={160} position="bottom-end">
                          ...
                          <Menu.Dropdown>
                            <Menu.Item leftSection={<IconEdit size={14} />} onClick={() => handleOpenEdit(w)}>
                              Sửa
                            </Menu.Item>
                            <Menu.Item leftSection={<IconTrash size={14} />} color="red" onClick={() => handleDelete(w)}>
                              Xóa
                            </Menu.Item>
                          </Menu.Dropdown>
                        </Menu>
        ...
        <Stack gap="md">
          <Group grow>
            <TextInput
              ...
              required
              value={formData.word}
              onChange={(e) => setFormData({ ...formData, word: e.currentTarget.value })}
            />
            <TextInput
              label="Phiên âm"
              placeholder="/ˈæp.əl/"
              value={formData.phonetic}
              onChange={(e) => setFormData({ ...formData, phonetic: e.currentTarget.value })}
            />
          </Group>
          <Group grow>
            <Select
              ...
              onChange={(v) => setFormData({ ...formData, partOfSpeech: (v || undefined) as PartOfSpeech | undefined })}
              clearable
          ...
          <TextInput
            ...
            onChange={(e) => setFormData({ ...formData, meaning: e.currentTarget.value })}
          />
          <TextInput
            label="Ví dụ"
            placeholder="VD: I eat an apple every day."
            value={formData.example}
            onChange={(e) => setFormData({ ...formData, example: e.currentTarget.value })}
          />
          <Group grow>
            <TextInput
              label="Audio URL"
