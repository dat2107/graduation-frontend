...
import type { GrammarTopicManage, CreateGrammarTopicRequest } from '@/@types/grammar'
...
const TeacherGrammarPage = () => {
  const navigate = useNavigate()
  const [topics, setTopics] = useState<GrammarTopicManage[]>([])
  ...
  const [editingTopic, setEditingTopic] = useState<GrammarTopicManage | null>(null)
  const [formLoading, setFormLoading] = useState(false)
  const [formData, setFormData] = useState<CreateGrammarTopicRequest>({
  ...
  const fetchTopics = useCallback(async () => {
    setLoading(true)
    try {
      const resp = await GrammarService.getTopicsForManage({
        page: page - 1,
        size: PAGE_SIZE,
  ...
  const handleOpenEdit = (t: GrammarTopicManage) => {
    setEditingTopic(t)
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingTopic) {
        await GrammarService.updateTopic(editingTopic.id, formData)
        setAlert({ type: 'success', message: `Đã cập nhật chủ đề "${formData.name}"` })
      } else {
        await GrammarService.createTopic(formData)
  ...
  // ─── Actions ───────────────────────────────────────────────────────────────
  const handleToggleActive = async (t: GrammarTopicManage) => {
    setActionLoading(t.id)
    try {
      await GrammarService.updateTopic(t.id, { active: !t.active })
  ...
  const handleDelete = async (t: GrammarTopicManage) => {
    setActionLoading(t.id)
    try {
  ...
  return (
    <Box p="lg">
      ...
      <Group justify="space-between" mb="lg">
        <Stack gap="xs">
          <Title order={2}>Quản lý Ngữ pháp</Title>
          <Text c="dimmed" size="sm">Tạo và quản lý các chủ đề ngữ pháp, thêm bài học vào từng chủ đề.</Text>
        </Stack>
      ...
      <SimpleGrid cols={{ base: 1, xs: 3 }} mb="lg">
        ...
        <Card withBorder radius="md" p="md">
          <Group>
            ...
            <div>
              <Text size="xs" c="dimmed">Tổng bài học</Text>
              <Text fw={700} size="lg">
          ...
          <Table striped highlightOnHover verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th ta="center">Số bài học</Table.Th>
                        ...
                        <Menu shadow="md" width={180} position="bottom-end">
                          ...
                          <Menu.Dropdown>
                            <Menu.Item
                              ...
                              onClick={() => navigate(`/teacher/grammar/${t.id}`)}
                            >
                              Xem bài học
                            </Menu.Item>
                            <Menu.Item
      ...
      >
        <Stack gap="md">
          <TextInput
            label="Tên chủ đề"
            placeholder="VD: Present Tenses, Conditionals..."
            required
            value={formData.name}
