...
import type { ListeningTopicManage, CreateListeningTopicRequest } from '@/@types/listening'
...
const TeacherListeningPage = () => {
  const navigate = useNavigate()
  const [topics, setTopics] = useState<ListeningTopicManage[]>([])
  ...
  const [editingTopic, setEditingTopic] = useState<ListeningTopicManage | null>(null)
  const [formLoading, setFormLoading] = useState(false)
  const [formData, setFormData] = useState<CreateListeningTopicRequest>({
    name: '', description: '', level: 'A1', emoji: '',
  })
  ...
  const fetchTopics = useCallback(async () => {
    setLoading(true)
    try {
      const resp = await ListeningService.getTopicsForManage({
        page: page - 1,
        size: PAGE_SIZE,
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({ name: '', description: '', level: 'A1', emoji: '' })
    openModal()
  }

  const handleOpenEdit = (t: ListeningTopicManage) => {
    setEditingTopic(t)
    setFormData({ name: t.name, description: t.description, level: t.level, emoji: t.emoji })
    openModal()
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingTopic) {
        await ListeningService.updateTopic(editingTopic.id, formData)
        setAlert({ type: 'success', message: `Đã cập nhật chủ đề "${formData.name}"` })
      } else {
        await ListeningService.createTopic(formData)
  ...
  // ─── Actions ───────────────────────────────────────────────────────────────
  const handleToggleActive = async (t: ListeningTopicManage) => {
    setActionLoading(t.id)
    try {
      await ListeningService.updateTopic(t.id, { active: !t.active })
  ...
  const handleDelete = async (t: ListeningTopicManage) => {
    setActionLoading(t.id)
    try {
      await ListeningService.deleteTopic(t.id)
  ...
  return (
    <Box p="lg">
      ...
      <Group justify="space-between" mb="lg">
        <Stack gap="xs">
          <Title order={2}>Quản lý Luyện nghe</Title>
          <Text c="dimmed" size="sm">Tạo và quản lý các chủ đề nghe, thêm bài nghe vào từng chủ đề.</Text>
        </Stack>
      ...
      <SimpleGrid cols={{ base: 1, xs: 3 }} mb="lg">
        <Card withBorder radius="md" p="md">
          <Group>
            <IconHeadphones size={24} color="var(--mantine-color-blue-6)" />
            <div>
        ...
        <Card withBorder radius="md" p="md">
          <Group>
            <IconHeadphones size={24} color="var(--mantine-color-violet-6)" />
            <div>
              <Text size="xs" c="dimmed">Tổng bài nghe</Text>
              <Text fw={700} size="lg">
                {loading ? <Skeleton width={30} height={20} /> : topics.reduce((s, t) => s + t.lessonCount, 0)}
              </Text>
            </div>
          </Group>
        </Card>
          ...
          <Table striped highlightOnHover verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th ta="center">Số bài nghe</Table.Th>
              ...
              {loading ? (
                ...
                topics.map((t) => (
                  <Table.Tr key={t.id} style={!t.active ? { opacity: 0.5 } : undefined}>
                    ...
                    <Table.Td ta="center">
                      <Text size="sm" fw={500}>{t.lessonCount}</Text>
                    </Table.Td>
                    <Table.Td ta="center">
                        ...
                        <Menu shadow="md" width={180} position="bottom-end">
                          ...
                          <Menu.Dropdown>
                            <Menu.Item
                              ...
                              onClick={() => navigate(`/teacher/listening/${t.id}`)}
                            >
                              Xem bài nghe
                            </Menu.Item>
                            <Menu.Item
      ...
      >
        <Stack gap="md">
          <TextInput
            label="Tên chủ đề"
            placeholder="VD: Daily Conversations, News..."
            required
            value={formData.name}
          ...
          {editingTopic && (
            <Switch
              ...
              checked={(formData as any).active ?? editingTopic.active}
...
export default TeacherListeningPage
