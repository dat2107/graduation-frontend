...
import type { GrammarTopic, GrammarLesson, GrammarProgress } from '@/@types/grammar'
...
export default function GrammarPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const [topicsRes, progressRes] = await Promise.all([
        GrammarService.getTopics(level),
  ...
  const handleTopicClick = async (topic: GrammarTopic) => {
    setSelectedTopic(topic)
    setLessonsLoading(true)
    open()
    const res = await GrammarService.getLessons({ topicId: topic.id })
    ...
    <Stack gap="xl" p="md">
      ...
      <div>
        ...
        <Text c="dimmed" size="sm" mt={4}>
          Học ngữ pháp tiếng Anh qua bài học và bài tập tương tác
        </Text>
      </div>
      ...
      <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="sm">
        ...
        <StatCard
          ...
          value={progress?.totalExercisesDone ?? 0}
          color="violet"
        />
      </SimpleGrid>
